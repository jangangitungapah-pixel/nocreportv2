...
f5_effects = timer_anchor + """  useEffect(() => {
  ...
  }, [duplicateFingerprint, localDevelopmentMode, routeTicketId, ticket]);
...
replace_once(
    ...
    "  const persistCoreTicket = async (values, { createAnyway = false } = {}) => {\n    if (persistPending) return;\n\n    if (\n      !routeTicketId &&\n      !localDevelopmentMode &&\n      duplicateCandidates.length > 0 &&\n      !duplicateAcknowledged &&\n      !createAnyway\n    ) {\n      pushToast({\n        title: 'Review possible duplicate',\n        message: 'Review the candidate Ticket or choose Create anyway before creating a new Ticket.',\n        tone: 'warning',\n      });\n      return;\n    }",
    'create duplicate gate',
)

replace_once(
    "  const submitTicket = handleSubmit(persistCoreTicket, notifyInvalidForm);",
    "  const submitTicket = handleSubmit((values) => persistCoreTicket(values), notifyInvalidForm);\n  const createAnyway = async () => {\n    const values = await validateFormState();\n    if (!values) return;\n    setDuplicateAcknowledged(true);\n    await persistCoreTicket(values, { createAnyway: true });\n  };",
    'create anyway handler',
)
